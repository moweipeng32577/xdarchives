 ...
 */
@Controller
@RequestMapping(value = "/digitalInspection")
public class DigitalInspectionController {
	...
	public String acquisition(Model model, String isp) {
		return "/inlet/digitalInspection";
	}
	...
	public Page<Szh_batch_bill> getBatchBillBySearch(String status, int page, int limit, String sort, String condition, String operator, String content){
		if("未抽检".equals(status)||"完成抽检".equals(status)){
			return digitalInspectionService.getBatchBillBySearch("质检",status,page,limit,sort,condition,operator, content);
		}else{
			return digitalInspectionService.getBatchBillBySearchCheckUser("质检",status,page,limit,sort,condition,operator,content);
		}
	}

	/**
	 * 分页获取质检批次条目信息
	 * @param notMe 组内抽检非本人标识
	 * @param batchcode 批次号
	...
	public Page<Tb_entry_index_capture> findByCaptureSearch(String archivesType, int page, int limit, String condition, String operator, String content, String sort) {
		...
		return digitalInspectionService.findByCaptureSearch("质检",archivesType,page, limit, condition, operator, content, sortobj);
	}
	...
	public ExtMsg batchAddFormSubmit(Szh_batch_bill bill) {
		boolean status = digitalInspectionService.batchAddFormSubmit(bill,"质检");
	...
	@ResponseBody
	public ExtMsg acceptSubmit(String[] batchcodes, String pages){
