Ext.define('FindAccount.view.ElectronicView', {
    entryid: undefined,     //条目主键ID
    entrytype: '',   //数据类型（采集、管理、利用）

    extend: 'Ext.panel.Panel',
    xtype: 'FindAccountElectronic',
    layout: 'border',
    bodyBorder: false,
    defaults: {
        split: true
            ...
            items: [
                {
                    ...
                    var view = this.findParentByType('FindAccountElectronic');
        ...
        XD.confirm('确定要删除这' + records.length + '条数据吗', function () {
            ...
            Ext.Ajax.request({
                ...
                success: function (response) {
                    ...
                    //修改user表accessory字段（附件）
                    Ext.Ajax.request({
                        method: 'post',
                        url: '/findAccount/setAccessory',
                        scope: this,
                        params: {
                            loginname: window.wform.down('[itemId = certificatenumberId]').getValue(),
                            eleids:treeEleids.join(',')
                        },
                        success: function (response) {
                            window.wmedia= treeEleids.join(',');
                        }
                    });
...
Ext.define('Comps.view.UploadView', {
    ...
    listeners: {
                ...
                Ext.Ajax.request({
                    ...
                    success: function (response, opts) {
                        ...
                        if (!win.destroyed) {
                            ...
                            grid.getView().getRow(record).uploadstate ='uploadsuccess'
                            record.set('eleid', data.eleid);
                            window.wmedia=window.wmedia==''?data.eleid:window.wmedia+","+data.eleid;

                            var  uploadState = [] ;//上传处理完成标识符
                            var storeDate = grid.getStore().getData().items;
                            for (var i = 0; i < storeDate.length; i++) {
                                uploadState.push(grid.getView().getRow(storeDate[i]).uploadstate);
                            }
                            var isUploadsuccess =false;
                            for (var i = 0; i < uploadState.length; i++) {
                                if (uploadState[i] == 'uploadsuccess') {
        ...
        beforeclose: function (win) {
            ...
            WebUploader.Uploader.unRegister('findAccountElectronic');
        }
    }
});