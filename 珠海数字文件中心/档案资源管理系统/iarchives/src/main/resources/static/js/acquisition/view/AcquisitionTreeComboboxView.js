 ...
 */
Ext.define('Acquisition.view.AcquisitionTreeComboboxView', {
    extend: 'Ext.form.field.Picker',
    xtype:'acquisitionTreeComboboxView',
        ...
        self.picker.on({
            itemclick: function (view, record, item, index, e, object) {
                ...
                if (!self.multiSelect) {
                    ...
                    var roottype=record.get('roottype');//节点类型，unit是单位
                ...
                var acquisitionfiling = self.findParentByType('acquisitionfiling');
                if(acquisitionfiling){
                    var filingGridView = acquisitionfiling.down('[itemId=filingSecondStep]').down('entrygrid');
                    filingGridView.nodeid = originalrecord.get('fnid');
                    var dynamicfilingform = acquisitionfiling.down('dynamicfilingform');
                    dynamicfilingform.removeAll();//重新加载动态表单前先清空form
                    dynamicfilingform.nodeid = originalrecord.get('fnid');
                    dynamicfilingform.initFilingField();//重新加载动态表单
                }
                var acquisitionDismantleForm = self.findParentByType('acquisitionDismantleFormView');
                if(acquisitionDismantleForm){
                    acquisitionDismantleForm.nodeid = originalrecord.get('fnid');
                }
            }
        });
        return self.picker;
