...
Ext.define('SupervisionWork.view.SupervisionWorkElectronicView', {
    entryid: undefined,     //条目主键ID
    entrytype: '',   //数据类型（采集、管理、利用）

    extend: 'Ext.panel.Panel',
    xtype: 'supervisionWorkElectronicView',
    layout: 'border',
    bodyBorder: false,
    defaults: {
        split: true
    ...
    items: [{
        ...
        store: {
            ...
            proxy: {
                type: 'ajax',
                url: '/electronic/supervisionWork/tree/',
                reader: {
                    type: 'json',
                    expanded: false
                }
            ...
            items: [
                {
                    ...
                    var view = this.findParentByType('supervisionWorkElectronicView');
                    var win = Ext.create('Comps.view.UploadView', {
                        organid: view.organid,
                        selectyear: view.selectyear,
                        savetype: view.savetype,
                ...
                },
                {
                    ...
                    var workview = supervisionWorkView.down('[itemId=workview]');
                    var trees = view.down('treepanel').getRootNode().childNodes;
                    var mediaView;
                    if(view.savetype=='fillingname'){
                        supervisionWorkView.down('[itemId=fillingnamecount]').setText('共' + trees.length + '份');
                        mediaView = supervisionWorkView.down('[itemId=fillingnameId]');
                    }else if(view.savetype=='classplanname'){
                        supervisionWorkView.down('[itemId=classplannamecount]').setText('共' + trees.length + '份');
                        mediaView = supervisionWorkView.down('[itemId=classplannameId]');
                    }else if(view.savetype=='fundsfiles'){
                        supervisionWorkView.down('[itemId=fundsfilescount]').setText('共' + trees.length + '份');
                        mediaView = supervisionWorkView.down('[itemId=fundsfilesId]');
                    }else if(view.savetype=='setindex'){
                        supervisionWorkView.down('[itemId=setindexcount]').setText('共' + trees.length + '份');
                        mediaView = supervisionWorkView.down('[itemId=setindexId]');
                    }else if(view.savetype=='normativefilename'){
                        supervisionWorkView.down('[itemId=normativefilenamecount]').setText('共' + trees.length + '份');
                        mediaView = supervisionWorkView.down('[itemId=normativefilenameId]');
                    }
                    var mediatext = '';
                    ...
                    mediaView.setValue(mediatext);
                    supervisionWorkView.setActiveItem(workview);
                }}
            ]
        }],
        listeners: {
            beforeload: function () {
                var view = this.findParentByType('supervisionWorkElectronicView');
                this.getStore().proxy.url = '/electronic/supervisionWork/tree/'+ view.organid+'/' + view.selectyear + '/'+ view.savetype
            },
            ...
            itemclick: function (view, record, item, index, e, eOpts) {
                ...
                mediaFrame.setAttribute('src', '/electronic/media?entrytype=supervisionWork&eleid=' + record.get('fnid') + '&filetype=' + filename.substring(filename.lastIndexOf('.') + 1));
            }
        }
    }, {
        region: 'center',
    ...
    initData: function () {
        var mediaFrame = document.querySelectorAll('#mediaFrame')[0];
        if(mediaFrame){
    ...
    del: function () {
        ...
        XD.confirm('确定要删除这' + records.length + '条数据吗', function () {
            ...
            Ext.Ajax.request({
                method: 'DELETE',
                url: '/electronic/supervisionWork/' + eleids.join(","),
                scope: this,
    ...
    listeners: {
        render: function (win) {
            ...
            win.uploader = WebUploader.create({
                ...
                swf: '/js/Uploader.swf',
                // 文件接收服务端。
                server: '/electronic/electronicsSupervisionWork',
                // 选择文件的按钮。可选。
            ...
            win.uploader.on('uploadSuccess', function (file, response) {
                Ext.Ajax.request({
                    method: 'POST',
                    url: '/electronic/electronics/supervisionWork/' + win.organid+'/' + win.selectyear + '/'+ win.savetype + '/'+file.name + '/',
                    success: function (response, opts) {
        ...
        beforeclose: function (win) {
            ...
            WebUploader.Uploader.unRegister('supervisionWorkElectronicView');
        }
    }
});
