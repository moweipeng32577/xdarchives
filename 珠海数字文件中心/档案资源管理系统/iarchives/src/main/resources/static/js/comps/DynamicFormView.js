 ...
 */
Ext.define('Comps.view.DynamicFormView',{
    ...
    initField:function(obj,operate){
        ...
        for(var i=0; i < obj.length; i++){
            if(!obj[i].inactiveformfield){
                activeFormField.push(this.getConvertedField(operate,obj[i]));
            }else{
                inactiveFormField.push(this.getConvertedField(operate,obj[i]));
            }
        }
        this.add(activeFormField);
        if(inactiveFormField.length>0){
            this.add({
                layout : 'column',
                xtype:'fieldset',
    ...
    getConvertedField:function (operate,template) {
        var field = {
            ...
            columnWidth: template.frows >= 1 ? (1-0.02) : (1-0.04),
            xtype:'textfield',
            margin:'1 1 1 1'
        };
    ...
    convertField:function (operate,template,field) {//根据不同的类型，返回不同的控件
        ...
        var displayfieldField = {//预留空间(*或'')
            ...
            margin:'2 1 1 1',
            value: '',
            columnWidth:template.frows >= 1 ? 0.02 : 0.04
        };
        ...
        switch (template.ftype) {
            case 'keyword' :
                //主题词标引，先添加文本框
                field.columnWidth = template.frows >= 1 ? (1-0.08-0.08-0.02) : (1-0.16-0.16-0.04);
                ...
                fieldArr.push({
                    ...
                    padding:'3.5',
                    scope:this,
                    disabled: operate == 'show' ? false : true,
            ...
            case 'string' :
                if(template.fieldcode == 'pages') {
                    field.columnWidth = template.frows >= 1 ? (1-0.08-0.02) : (1-0.16-0.04);
                    field.margin = '1 0 1 1';
                    field.regex = /^\d+$/;
            ...
            case 'calculation' :
                ...
                field.listeners={
                    blur: function(){
                        ...
                        handler: {
                            var form = this.up("dynamicform");//指向当前表单
                            var archivecodeField = form.getForm().findField('archivecode');//获取到表单中档号框的值
                            var archivecode = archivecodeField.value;
                            var oldArchivecode=archivecodeField.originalValue;
                            if(typeof(archivecode) != 'undefined' && archivecode != null && archivecode != ''){//如果获取到的档号选择的值不为null
                                var lastValue=this.value;
                                var oldValue=this.originalValue;
                                if(typeof(lastValue) != 'undefined' && lastValue != null && lastValue != ''){
                                    //获取最后一个档号分隔符
                                    if(oldArchivecode!=""){//修改前是有档号值的
                                        var splitCode=oldArchivecode.substring(oldArchivecode.lastIndexOf(oldValue)-1,oldArchivecode.lastIndexOf(oldValue))
                                        archivecode=archivecode.substring(0,archivecode.lastIndexOf(splitCode)+1)+lastValue;//截取掉分隔符后边的计算项编号后加上新的计算项编号
                                    }else{//修改前是没有档号值的,档号字段的末尾直接替换计算项编号
                                        archivecode=archivecode.substring(0,archivecode.length-lastValue.length)+lastValue;
                                    }
                                    archivecodeField.setValue(archivecode);
                                }
                            }
                        }
                    }
    ...
    getPages:function() {
        ...
        var dynamicFrom = this.ownerCt.down('dynamicform');
                ...
                success: function (response) {
                    if (Ext.decode(response.responseText).success == false) {
                        if (Ext.decode(response.responseText).data != pagesfield.getValue()) {
                            if(dynamicFrom.editType=='archiveEdit'){//预归档页面获取页数，直接写上
    ...
    setArchivecodeValueWithNode:function(nodename){
        ...
        for(var i=0;i<formfield.length;i++){
            ...
            if(fdlength!=null&&fdlength!=''){
                ...
                if(formtextfield!=null && formtextfield.getValue()!=null){
        ...
        fields.forEach(function (item,index,arr) {
            if(item.xtype =='textfield' || item.xtype =='textarea'){//只处理输入框的数据
                if(item.lastValue!==''){
                    var trimStr = item.lastValue.replace(/^\s+|\s+$/g,'');
                    item.setValue(trimStr);
                }
            }
        });
        var formValues = form.getValues();
    ...
    fileLabelStateChange:function (view,operate) {
        if(operate == 'look'||operate == 'lookfile'){
            ...
            //     //查看条目需要隐藏所有按钮
            //     var btn=  view.down('toolbar').query('button')[i];
            // }
            this.getELetopBtn(view.down('toolbar').query('button'),operate);
        }else{
            for(var i=0;i<view.down('toolbar').query('button').length;i++){
                var btn = view.down('toolbar').query('button')[i];
            }
    ...
    //获取表单字段
    getFormField:function (xtType) {
        var formField;
        var url = '/template/form';
        if(xtType=='声像系统'){
            url = '/template/sxform';
        }
        Ext.Ajax.request({
    ...
    //获取声像表单字段
    getSxFormField:function (nodeName) {
        var formField;
        Ext.Ajax.request({
            url: '/template/sxform',
            async:false,
    ...
    getELetopBtn:function (btns,operate) {
        var btnFunctionList = [];
        if(typeof(wjqxFunctionButton) !== 'undefined'&&wjqxFunctionButton){
            btnFunctionList =wjqxFunctionButton.data;
        }else{
            Ext.Ajax.request({
                url: '/user/getWJQXbtn',
                async:false,
        ...
        for(var i = 0;i<btns.length;i++) {
            ...
            if (operate == 'look' || operate == 'lookfile') {
                if (btn.text == '删除' || btn.text == '上传' || btn.text == '上移' || btn.text == '下移' || btn.text == '查看历史版本') {
                    btn.hide();
                }
            }
            if (operate == 'add') {
