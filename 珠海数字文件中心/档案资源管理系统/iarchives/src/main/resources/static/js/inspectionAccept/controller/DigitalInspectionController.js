Ext.define('DigitalInspection.controller.DigitalInspectionController', {
    ...
    views: [
        ...
        'DigitalInspectionExpMessageView',
        'EntryFormView'
    ],//加载view
    stores: [
    ...
    init: function () {
        ...
        this.control({
            ...
            'DigitalInspectionWcAcceptGridView [itemId=export]':{//成果导出
                click:this.resultsExport
            },

            'DigitalInspectionExpMessageView [itemId=SaveExport]':{
                click:this.SaveExport
            },

            'DigitalInspectionExpMessageView [itemId=cancelExport]':{
                click:function (btn) {
                    btn.up('DigitalInspectionExpMessageView').close();
                }
            }
        });
    },
    ...
    changeBatchEntry:function(entryId){
        ...
        Ext.Ajax.request({
            url: '/inspectionAccept/changeBatchEntry',
            method: 'POST',
...
//                form.formStateChange(operate);
    ...
    addBatch:function(btn){
      ...
      var batchAddForm = batchAddView.down('form');
        batchAddForm.load({
          url: '/inspectionAccept/getBatchAddForm',
        success: function (form, action) {
        },
        failure: function () {
            XD.msg('获取批次号失败');
    ...
    entryImport:function(btn,billWclGrid){
        ...
        batchAddForm.submit({
            url: '/inspectionAccept/batchAddFormSubmit',
            method: 'POST',
            success: function (response) {
                XD.msg('导入成功');
    ...
    samplingSubmit:function(btn){
        ...
        if(form.isValid()){
            ...
            form.submit({
                url: '/inspectionAccept/samplingSubmit',
                method: 'POST',
                success: function () {
                    XD.msg('设置抽检率完成');
    ...
    cancelSampling:function(btn){
        ...
        XD.confirm('确定要退回选择的'+record.length+'条批次吗?',function(){
            Ext.Ajax.request({
                params: {'batchcodes': batchcodes},
                url: '/inspectionAccept/cancelSampling',
                method: 'POST',
                sync: true,
                success: function (resp) {
                    var respText = Ext.decode(resp.responseText);
    ...
    initSamplingDetail:function(detailView,code){
        ...
        detailEntryGrid.getStore().proxy.extraParams.batchcode = batchcode;
        detailEntryGrid.getStore().reload();
    ...
    samplingDetailMsg:function(view, record, item, index, e){
            ...
            success: function (resp) {
                ...
                for(var i in respText){
                    ...
                    if(!isNaN(i)&&types.indexOf(type)>-1){
                        if(!state){
                            // p1.changeImg('/inspectionAccept/showMedia?eleid='+respText[i].eleid,respText[i].medianame);
        ...
        entryForm.load({
            url: '/inspectionAccept/getFormEntry',
            method: 'POST',
            params: {
                'entryid': entryId
            },
    ...
    showMetadata:function(mediaid){
        ...
        metadataForm.load({
            url: '/inspectionAccept/getMetadata',
            method: 'POST',
            params: {
                'mediaid': mediaid
            },
    ...
    changeMediaStatus:function(){
        ...
        if(rowRecord){
            if(rowRecord.data.status=='未检查'){
                Ext.Ajax.request({
                    ...
                    url: '/inspectionAccept/changeMediaStatus',
                    method: 'POST',
                    sync: true,
                    success: function (resp) {
                        var respText = Ext.decode(resp.responseText);
        ...
        p1.changeImg('/inspectionAccept/showMedia?eleid='+detailView.selMediaId,rowRecord.data.medianame);
    ...
    photoPass:function () {
        ...
        XD.confirm('确定通过吗?',function(){
            Ext.Ajax.request({
                params: {'id':billEntryId,'batchcode':batchcode},
                url: '/inspectionAccept/passEntry',
                method: 'POST',
                sync: true,
                success: function (resp) {
                    var respText = Ext.decode(resp.responseText);
    ...
    errSubmit:function (btn) {
        ...
        var that = this;
        form.submit({
            url: '/inspectionAccept/errSubmit',
            method: 'POST',
            success: function () {
                XD.msg('操作成功');
    ...
    errDel:function(){
        ...
        XD.confirm('确定要删除选择的'+errids.length+'条数据吗?',function(){
            Ext.Ajax.request({
                ...
                url: '/inspectionAccept/delMediaErrs',
                method: 'POST',
                success: function (resp) {
                    var respText = Ext.decode(resp.responseText);
                    XD.msg(respText.msg);
    ...
    errRepair:function(){
        ...
        XD.confirm('确定要修复选择的'+errids.length+'条数据吗?',function(){
            Ext.Ajax.request({
                params: {'errids': errids},
                url: '/inspectionAccept/errRepair',
                method: 'POST',
                success: function (resp) {
                    var respText = Ext.decode(resp.responseText);
                    if(respText.success){
    ...
    errExport:function(){
        var batchcode = detailView.batchcodes[0];
        location.href =  '/inspectionAccept/exportErrReport?batchcode='+batchcode;
    },
    ...
    metadataSave:function (btn) {
        ...
        metadataForm.submit({
            method: 'POST',
            url: '/inspectionAccept/metadataSubmit',
            scope: this,
            success: function (form, action) {
                XD.msg('修改成功');
            },
    ...
    accept:function(btn){
        ...
        var entryTabGrid = grid.up('DigitalInspectionWcView').down('DigitalInspectionWcSouthView').down('entrygrid');
        var select = grid.getSelectionModel();
        if (select.getSelected().length<1) {
            XD.msg('至少选择一个批次');
            return;
        ...
        XD.confirm('确定要验收选择的'+batchcodes.length+'条批次吗?',function(){
            Ext.Ajax.request({
                params: {'batchcodes':batchcodes},
                url: '/inspectionAccept/acceptSubmit',
                method: 'POST',
                success: function (resp) {
                    var respText = Ext.decode(resp.responseText);
                    if(respText.success){
    ...
    checkMd5:function (btn) {
        ...
        //     return;
        // }
        Ext.MessageBox.wait('正在检测该批次数字化成果，请稍后...', '提示');
        setTimeout(function () {
            var win = Ext.create('Ext.window.Window',{
                ...
                items:[{
                    title:'准确性',
                    itemId:'authenticity',
                    html:'<span style="color:green">验证通过：</br></br>核对元数据信息通过</br></br>核对EXIF信息通过</span>',
                    flex: 1
                },{
                    title:'完整性',
                    itemId:'integrity',
                    html:'<span style="color:green">验证通过：</br></br>目录检查通过</br></br>元数据检查通过</br></br>图像检查通过</span>',
                    flex: 1
                },{
                    title:'可用性',
                    itemId:'usability',
                    ...
                    title:'MD5校验',
                    html:'<span style="color:green">验证通过：</br></br>MD5码校验通过</span>',
                    flex: 1
                }],
                buttons:[{
                    text:'关闭',
    ...
    resultsExport:function(btn){
        var grid = btn.up('DigitalInspectionWcAcceptGridView');
        var select = grid.getSelectionModel();
        if (select.getSelected().length<1) {
            XD.msg('至少选择一个批次');
            return;
        ...
        for (var i = 0; i < record.length; i++) {
         ...
         var expFieldSelView = Ext.create('DigitalInspection.view.DigitalInspectionExpMessageView',{'batchcodes':batchcodes});
         expFieldSelView.show();
        // XD.msg('服务器繁忙,请稍后再试...');
    },

    SaveExport:function(btn){
        var messgeView = btn.up('DigitalInspectionExpMessageView');
        var fileName = messgeView.down('[itemId=userFileName]').getValue();
        var zipPassword = messgeView.down('[itemId=zipPassword]').getValue();
        Ext.MessageBox.wait('导出中...');
        Ext.Ajax.request({
            params: {'fileName': fileName,'zipPassword':zipPassword,'batchcodes':messgeView.batchcodes},
            url: '/inspectionAccept/chooseFieldExport',
            method: 'POST',
            timeout:XD.timeout,
            scope: this,
            async:true,
            success: function (resp) {
                ...
                if(obj.success){
                    window.location.href="/export/downloadZipFile?fpath="+encodeURIComponent(obj.data);
