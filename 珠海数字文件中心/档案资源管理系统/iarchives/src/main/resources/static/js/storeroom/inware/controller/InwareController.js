...
var NodeIdf = "";
var iniIds = "";
Ext.define('Inware.controller.InwareController',{
    ...
    'FormAndGridView','InwareFormView','ManagementEntryGridView','AdvancedSearchFormView','ViewInwareDetail',
    'ReturnMessageView','ExportDetailView'],
    ...
    init:function(){
        this.control({
            ...
            'wareFormView button[itemId=deleteAll]':{
                click:function(btn){
                    XD.confirm('确定删除全部已选的入库档案', function () {
            ...
            "inWareDetailView button[itemId=export]":{
            ...
            },'inWareDetailView [itemId=lookDetail]':{
                click:function (view) {
                    var grid = view.up('inWareDetailView');
                    ...
                    if(isSelectAll){
                        archivecodeArr.push("iniId-"+iniIds);
                    }
                    ...
                    var entityDetailView = new Ext.create('Inware.view.EntityDetailView');
    ...
    openExport:function (view) {
        var inWareDetailView = view.findParentByType('inWareDetailView');
        ...
        var exportMissView = Ext.create('Inware.view.ExportDetailView');
    ...
    saveHandler:function(btn){
        ...
        var kf = wareFormView.down('[itemId = roomId]');
        var mjj = wareFormView.down('[itemId = zoneId]');
        var colI = wareFormView.down('[itemId = colId]');
        ...
        formpanel.submit({
            ...
            success : function(form, action) {
                ...
                Ext.Ajax.request({
                    url:'/management/deleteInwares',//删除已添加待入库的条目
                    method: 'POST',
                    timeout:XD.timeout,
                    params:{
                        entryids:ids
                ...
                //打开密集架
                var col = this.PrefixZero(colI.rawValue, 2)
                XD.confirm('是否打开'+kf.rawValue+'-'+mjj.rawValue+'-'+col +'列?',function() {
                    Ext.Ajax.request({
                        method: 'POST',
                        url: '/deviceTask/openAssginColumnByZoneId',
                        params:{zoneid:mjj.getValue(),col:col},
                        success:function (response) {
        ...
        var win = this.getView('ManagementView').create({
            ...
            listeners: {
                close: function (_this) {
                    Ext.MessageBox.wait('正在读取选中的档案数据...')
                    Ext.Ajax.request({
...
//        }
    ...
    returnHandler:function(btn){
        ...
        //把数据entryid 提交入库
        XD.confirm('是否入库？', function () {
            var win = Ext.create("Inware.view.ReturnMessageView", {});
    ...
    addReturn:function(btn){
        ...
        Ext.MessageBox.wait('正在处理请稍后...');
        Ext.Ajax.request({
            params: {ids: entryids,remarkText:remarkValue,returnware:returnMan},
            ...
            success: function (response) {
                ...
                XD.msg('归还入库成功');
                messageView.close();
                window.returnGridView.initGrid({nodeid:templateNodeid});

                //打开密集架
                var shelves = Ext.decode(response.responseText);
                var zone = shelves.data.zone;
                var deviceid = zone.device;

                var col = this.PrefixZero(shelves.data.col, 2)
                XD.confirm('是否打开'+zone.roomdisplay+'-'+zone.zonedisplay+'-'+col +'列? （注：多条目入库时，默认打开第一条条目的密集架列！）',function() {
                    Ext.Ajax.request({
                        method: 'POST',
                        url: '/deviceTask/openAssginColumnByDeviceId',
                        params:{deviceid:deviceid,col:col},
                        success:function (response) {
                            var obj = Ext.decode(response.responseText);
                            XD.msg(obj.msg);
                        }
