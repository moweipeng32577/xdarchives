 ...
 */
Ext.define('Curator.controller.CuratorController', {
    ...
    stores: ['CuratorDclStore','CuratorYclStore','ProjectLogLookGridStore'],//加载store
        ...
        this.control({
            'curatorView':{
                tabchange:function (view) {//tab页面切换触发
                    ...
                    }else if(view.activeTab.title == '待处理'){
            ...
            'curatorYclView':{
                afterrender:function (view) {
                    view.initGrid({projectstatus:"领导审阅通过发布,领导审阅不发布"});
                }
            },
            'curatorDclView':{
                afterrender:function (view) {
                    view.initGrid({projectstatus:"副领导审阅通过,提交馆长审阅"});
                }
            },
            'curatorDclView button[itemId=look]':{  //待处理-查看
                click:function (view) {
                    ...
                    var projectAddLookView = Ext.create('Curator.view.ProjectAddLookView');
            ...
            'curatorDclView button[itemId=review]': {  //待处理-审阅
                click:function(view){
                    ...
                    var approveAddView = Ext.create('Curator.view.ApproveAddView',{ids:ids,grid:curatorDclGridView});
            ...
            'approveAddView button[itemId=approveAddSubmit]':{ //审阅-提交
                click:function(view){
                    ...
                    Ext.Ajax.request({
                        url:'/projectRate/updateProjectStatusByCurator',
                        method:'POST',
                        params:{
