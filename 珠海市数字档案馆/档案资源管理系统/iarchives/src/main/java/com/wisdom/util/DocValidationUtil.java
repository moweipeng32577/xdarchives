...
public class DocValidationUtil {
	...
	private static void getAllFileType() {
		...
		FILE_TYPE_MAP.put("323031", "txt"); // Real Audio (ram)
	}

	/**
	 * 比较文件头信息对应的扩展名与文件的扩展名
	 *
	 * @param extension
	 *            文件扩展名
	 * @param inputStream
	 *            文件数据流
	 * @return boolean true 文件头与扩展名相符合 false 文件头与扩展名不符合
	 */
	public static boolean contrastFileType(String extension, InputStream inputStream) {
	...
	public static String getFileType(InputStream inputStream) {
		...
		try {
			...
			while (keyIter.hasNext()) {
				...
				if (key.toUpperCase(Locale.ENGLISH).startsWith(fileCode.toUpperCase(Locale.ENGLISH))
						|| fileCode.toUpperCase(Locale.ENGLISH).startsWith(key.toUpperCase(Locale.ENGLISH))) {
	...
	// try {
	// File file = new File("D:\\学习\\培训文档\\富滇项目20180515更新后问题.docx");
	// FileInputStream inputStream = new FileInputStream(file);
	// contrastFileType("inputStream", inputStream);
	// } catch (IOException e) {
	// e.printStackTrace();
	// }
	// }
