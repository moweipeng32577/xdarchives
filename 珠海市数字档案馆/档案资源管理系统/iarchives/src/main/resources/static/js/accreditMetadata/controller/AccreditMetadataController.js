 ...
 */
Ext.define('AccreditMetadata.controller.AccreditMetadataController', {
    extend: 'Ext.app.Controller',

    views: ['AccreditMetadataView', 'AccreditMetadataGridView', 'AccreditMetadataPromptView'],//加载view
    stores: ['AccreditMetadataTreeStore', 'AccreditMetadataGridStore'],//加载store
    ...
    init: function () {
        ...
        this.control({
                    ...
                    Ext.Ajax.request({
                        ...
                        success: function (response) {
                            ...
                            if (respText.success == true) {
                                var win = new Ext.create('AccreditMetadata.view.AccreditMetadataWindow', {
                                    title: '增加参数',
            ...
            'accreditMetadataWindow button[itemId=save]': {
                click: function (view) {
                    ...
                    if (data['shortname'] == '' || data['text'] == '') {
                        XD.msg("有必填项未填写");
                        return;
                    }
                    var URL = '/accreditMetadata/addSystemConfig';
                    if (systemConfigWindow.title == '增加参数') {
                        ...
                        if(window.treesettingview.selection.get('fnid') == ''){
                            if(form.getForm().findField('shortname').isDirty()){
                            ...
                            if(form.getForm().findField('shortname').isDirty()||form.getForm().findField('shortname').isDirty()){
                    ...
                    if (systemConfigWindow.title == '增加参数') {
                        ...
                        if (window.treesettingview.selection.data.fnid=='') {
                            window.treesettingview.selection.insertChild(childNodes.length, {
                                text: respText.data.shortname,
                                leaf: true,
                                fnid: respText.data.aid,
        ...
        for (var i = 0; i < gridselections.length; i++) {
            configid.push(gridselections[i].get("aid"));
        }
        var configids = configid.join(',');
        Ext.Ajax.request({
            url: '/accreditMetadata/deletAccredit/' + configids,
            method: 'delete',
            sync: true,
            success: function (response) {
                var respText = Ext.decode(response.responseText);
                ...
                gridstore.loadPage(1, {//默认重新加载第一页
                    callback: function () {
                        var newnode = {
                            text: window.treesettingview.selection.get('shortname'),
                            leaf: true,//改变样式
