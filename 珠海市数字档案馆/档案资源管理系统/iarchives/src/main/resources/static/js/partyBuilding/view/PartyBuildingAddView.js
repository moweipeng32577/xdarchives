...
Ext.define('PartyBuilding.view.PartyBuildingAddView',{
    extend:'Ext.window.Window',
    xtype:'partyBuildingAddView',
    itemId:'partyBuildingAddViewId',
    frame: true,
    resizable: true,
    flag:'',
    width: '60%',
    ...
    items: [
        {
            ...
            // height:138,
            items: [
                {columnWidth: 1, fieldLabel: '', name: 'partybuildingID', hidden: true},
                            ...
                            render: function (view) {//渲染后添加双击事件
                                view.addListener("dblclick", function () {
                                    XD.confirm('确定要删除这个' + view.text + '文件吗', function () {
                                        ...
                                        Ext.Ajax.request({
                                            url: '/partyBuilding/electronics/'+view.eleid+ '/',
                                            method: 'DELETE',
                                            sync: true,
    ...
    listeners: {
        render: function (win) {
            ...
            win.uploader = WebUploader.create({
                ...
                swf: '/js/Uploader.swf',
                // 文件接收服务端。
                server: '/partyBuilding/electronicsPartyBuilding',
                // 选择文件的按钮。可选。
            ...
            win.uploader.on('uploadSuccess', function (file, response) {
                Ext.Ajax.request({
                    method: 'POST',
                    url: '/partyBuilding/electronics/'+ (typeof(win.informid) == 'undefined' ? '' : '/' + win.informid) + '/' + file.name + '/',
                    success: function (response, opts) {
        ...
        beforeclose: function (win) {
            ...
            WebUploader.Uploader.unRegister('partyBuilding');
        }
    }
});