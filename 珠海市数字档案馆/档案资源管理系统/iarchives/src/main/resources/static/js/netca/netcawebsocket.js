 ...
 */
function netca_signedDataSign(certEncode, selectType, selectCondition, tbs, _useSubjectKeyId, _useQ7, _tsaURL, _detached, _includeCertOption, _certPwd, SuccessSignedDataSignCallBack, FailedSignedDataSignCallBack,form) {
    var params = {};
    if (_certPwd == "") {
        params = {
            cert: { //证书(CertificateParams)
    ...
    NetcaPKI.signedDataSign(params)
        .Then(function (res) {
            if (res.result == -5) {
                ...
                SuccessSignedDataSignCallBack(res,form);
            }
 ...
 */
function netca_isInsertKey(certEncode, keySN, keyType, InsertCallBack, UnInsertCallBack,form) {
    var params = {
        cert: {
            "encode": certEncode
        },//可选,指定证书所在的Key是否插入
...
//获取客户端KeyX版本号相关信息
function netca_getClientVersionInfo(SuccessGetVersionInfoCallBack, FailedGetVersionInfoCallBack,form) {

    var params = {};
    NetcaPKI.getClientVersionInfo(params)
	    .Then(function (res) {
	        SuccessGetVersionInfoCallBack(res,form);
	    })
        .Catch(function (res) {
            FailedGetVersionInfoCallBack(res);
        });
