...
var dataSoure;
Ext.define('BatchModify.controller.BatchModifyController', {
    extend: 'Ext.app.Controller',
    xtype:'batchModifyController',
    views: [
    	'BatchModifyView','BatchModifyResultGridView',
        'BatchModifyPromptView','BatchModifyFormView',
        'BatchModifyModifyFormView','BatchModifyResultPreviewGrid',
        'BatchModifyReplaceFormView','BatchModifyAddFormView',
        'BatchModifyTreeView'
    ],
    models: [
        ...
        this.control({
            'batchModifyTreeView':{
                select: function (treemodel, record) {
                    ...
                    var batchModifyResultgrid = batchModifyView.down('batchModifyResultGridView');
            ...
            'batchModifyResultGridView button[itemId=batchModifyBackId]':{click:this.doAdvancedSearchBack},
            'batchModifyResultGridView button[itemId=batchModifyShowId]':{
                click:function(btn){
                    var batchModifyResultGridView = btn.findParentByType('batchModifyResultGridView');
                    var record = batchModifyResultGridView.selModel.getSelection();
                    if(record.length==0){
            ...
            'batchModifyResultGridView button[itemId=batchModifyModifyId]':{//结果列表界面　批量修改
            ...
            'batchModifyResultGridView button[itemId=batchModifyReplaceId]':{//结果列表界面　批量替换
            ...
            'batchModifyResultGridView button[itemId=batchModifyAddId]':{//结果列表界面　批量增加
    ...
    findGridView: function (btn) {
        return this.findView(btn).down('batchModifyResultGridView');
    },
    //切换到列表界面视图
    ...
    doAdvancedSearch:function (btn){//分类检索页面查询方法
        ...
        dataSoure=formParams['datasoure']=="management"?"数据管理":"数据采集";
        var grid = this.findView(btn).down('batchModifyResultGridView');
...
function deleteTempData() {//删除临时表Tb_entry_index_temp中的所有数据
    Ext.Ajax.request({
        method: 'DELETE',
        url: '/batchModify/tempEntryindex',
        success: function (response) {
        }
    })
}
...
function updateData(btn,batchtype) {
    ...
    var params = {
        ...
        flag:resultPreviewGrid.operateFlag,
        type:dataSoure,
        batchtype:batchtype //区分不同类型的批量操作
    };
