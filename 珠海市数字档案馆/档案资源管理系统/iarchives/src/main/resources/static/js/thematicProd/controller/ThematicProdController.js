...
Ext.define('ThematicProd.controller.ThematicProdController', {
    ...
    views: ['ThematicProdView',
        ...
        'ElectronicView',
        'SimpleSearchView',
        'SimpleSearchGridView','ThematicBackGridView'],//加载view
    stores: ['ThematicProdGridStore', 'ThematicProdDetailGridStore', 'SimpleSearchGridStore','ThematicBackGridStore'],//加载store
        ...
        this.control({
            ...
            'thematicProdGridView button[itemId=thematicAddBtnID]': {
                click: function (view, e, eOpts) {
                    ...
                    window.down('[itemId=component]').autoEl.src = "/thematicProd/getBackground?url=" + '/static/img/icon/thematic_def.png';
                    window.title = '增加';
                    window.show();
                }
            },
                    ...
                    if (records.length == 0) {
                        ...
                        XD.confirm('确定要删除这' + records.length + '条数据吗', function () {
                            ...
                            Ext.Ajax.request({
                                ...
                                url: '/thematicProd/deleteThematic',
                                method: 'POST',
                                sync: true,
            ...
            'thematicProdGridView button[itemId=releaseBtnID]': {
                click: function (view, e, eOpts) {
                    ...
                    if (records.length == 0) {
                        ...
                        var ispass = true;
                        for (var i = 0; i < records.length; i++) {
                            if (records[i].get('publishstate') != '已通过'){
                                ispass = false
                            }
                            thematicids.push(records[i].get("thematicid"));
                        }
                        if (!ispass){
                            XD.msg('发布失败，发布内容必须是已通过的专题！');
                            return;
                        }
                        XD.confirm('确定要发布这' + records.length + '条数据吗', function () {
                            ...
                            Ext.Ajax.request({
                                ...
                                url: '/thematicProd/releaseThematicStuats',
                                method: 'POST',
                                timeout:XD.timeout,
                                sync: true,
                                success: function (resp, opts) {
            ...
            'thematicProdGridView button[itemId=resultSubmit]': {//成果提交
                click: function (view, e, eOpts) {
                    var thematicProdGridView = view.findParentByType('thematicProdGridView');
                    var select = thematicProdGridView.getSelectionModel();
                    var records = select.getSelection();
                    if (records.length == 0) {
                        ...
                        XD.confirm('确定要提交这' + records.length + '条数据吗', function () {
                            var thematicids = [];
                            for (var i = 0; i < records.length; i++) {
                                if(records[i].get("publishstate") == '已发布'){
                                    XD.msg('已发布的专题不能进行成果提交');
                                    return;
                                }
                                thematicids.push(records[i].get("thematicid"));
                            }
                            Ext.Msg.wait('正在进行提交操作，请耐心等待……','提示');
                            Ext.Ajax.request({
                                params: {thematicids: thematicids.join(',')},
                                url: '/thematicProd/resultSubmit',
                                method: 'POST',
                                timeout:XD.timeout,
                                sync: true,
                                success: function (resp, opts) {
            ...
            'thematicProdGridView button[itemId=lookbacklog]': {//查看退回记录
                click:this.lookbacklog
            },
                ...
                click: function (view, e, eOpts) {
                    ...
                    if (records.length == 0) {
                        ...
                        XD.confirm('确定要取消发布这' + records.length + '条数据吗', function () {
                            ...
                            Ext.Ajax.request({
                                params: {thematicids: thematicids.join(',')},
                                url: '/thematicProd/cancleReleaseThematic',
                                method: 'POST',
                                sync: true,
                                success: function (resp, opts) {
                                    XD.msg(Ext.decode(resp.responseText).msg);
            ...
            'thematicProdDetailAddWindow': {
                ...
                //     field.down("[itemId=sectionId]").on('keyup', function (ob) {
                //         validaSplitCode(ob);
                //     });
                //     field.down("[itemId=chapterId]").on('keyup', function (ob) {
                //         validaSplitCode(ob);
                //     });
                // }
            },
            ...
            'simpleSearchView [itemId=simpleSearchSearchfieldId]': {
                search: function (searchfield) {
                    ...
                    grid.dataParams = {
                        ...
                        isCompilationManageSystem:true //是否为编研管理系统
                    };
            ...
            'EntryFormView [itemId=back]': {
                ...
                click:function (btn) {
                    ...
                    XD.confirm('确定要发布这'+records.length+'条专题吗?', function () {
                        Ext.MessageBox.wait('正在发布请稍后...', '提示');
                        location.href = '/thematicProd/releaseThematic?thematicids='+thematicIds;
                        // Ext.Ajax.request({
                        //     params: {thematicids: thematicIds},
                        //     url: '/thematicProd/releaseThematic',
                        //     method: 'POST',
                        //     sync: true,
                        //     success: function (resp, opts) {
                                 Ext.MessageBox.hide();
...
//                form.formStateChange(operate);
    ...
    lookbacklog:function (btn) {//查看退回记录
        var thematicBackGridView = Ext.create('Ext.window.Window',{
            modal:true,
            width:1000,
            height:530,
            title:'退回记录',
        ...
        var store = thematicBackGridView.down('thematicBackGridView').getStore();
        store.proxy.extraParams.state = "已退回";
        store.reload();
        thematicBackGridView.show();
    }
});
...
function validaSplitCode(splitcode) {
    ...
    //for(var i=0;i<str.length;i++) {
        ...
        if (result){
            XD.msg("含有不支持的特殊字符！");
            splitcode.setValue(str.substring(0,str.length-1));
            return false;
        } else {
            return true;
        }
